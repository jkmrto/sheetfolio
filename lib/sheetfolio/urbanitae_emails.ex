defmodule Sheetfolio.UrbanitaeEmails do
  ...
  `Sheetfolio.UrbanitaeParser`. Nothing is cached or written — the events are
  a cross-check for the screenshot-fed `urbanitae_transactions` collection,
  not a replacement for it (see `mix parse_urbanitae_emails`).
  """
  ...
  def fetch_all(progress \\ nil) do
    with {:ok, messages} <- GmailClient.search_messages(@gmail_query) do
      ...
      {events, ignored} =
        ...
        |> Enum.reduce({[], 0}, &collect/2)

      {:ok, {Enum.sort_by(events, & &1.date), ignored}}
    end
  end
